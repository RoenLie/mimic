/* @web/test-runner snapshot v1 */
export const snapshots = {};

snapshots["vaadin-month-calendar host default"] = 
`<vaadin-month-calendar aria-hidden="true">
</vaadin-month-calendar>
`;
/* end snapshot vaadin-month-calendar host default */

snapshots["vaadin-month-calendar shadow default"] = 
`<div
  aria-hidden="true"
  id="month-header"
  part="month-header"
>
  February 2016
</div>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat
  as="week"
  style="display: none;"
>
  <template is="dom-repeat">
  </template>
</dom-repeat>
<table
  aria-labelledby="month-header"
  id="monthGrid"
  role="grid"
>
  <thead id="weekdays-container">
    <tr
      part="weekdays"
      role="row"
    >
      <th
        aria-hidden="true"
        hidden=""
        part="weekday"
      >
      </th>
      <th
        abbr="Sunday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Sun
      </th>
      <th
        abbr="Monday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Mon
      </th>
      <th
        abbr="Tuesday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Tue
      </th>
      <th
        abbr="Wednesday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Wed
      </th>
      <th
        abbr="Thursday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Thu
      </th>
      <th
        abbr="Friday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Fri
      </th>
      <th
        abbr="Saturday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Sat
      </th>
    </tr>
  </thead>
  <tbody id="days-container">
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        5
      </td>
      <td
        aria-label=""
        part="date"
        role="gridcell"
        tabindex="-1"
      >
      </td>
      <td
        aria-label="1 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        1
      </td>
      <td
        aria-label="2 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        2
      </td>
      <td
        aria-label="3 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        3
      </td>
      <td
        aria-label="4 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        4
      </td>
      <td
        aria-label="5 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        5
      </td>
      <td
        aria-label="6 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        6
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        5
      </td>
      <td
        aria-label="7 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        7
      </td>
      <td
        aria-label="8 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        8
      </td>
      <td
        aria-label="9 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        9
      </td>
      <td
        aria-label="10 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        10
      </td>
      <td
        aria-label="11 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        11
      </td>
      <td
        aria-label="12 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        12
      </td>
      <td
        aria-label="13 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        13
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        6
      </td>
      <td
        aria-label="14 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        14
      </td>
      <td
        aria-label="15 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        15
      </td>
      <td
        aria-label="16 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        16
      </td>
      <td
        aria-label="17 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        17
      </td>
      <td
        aria-label="18 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        18
      </td>
      <td
        aria-label="19 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        19
      </td>
      <td
        aria-label="20 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        20
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        7
      </td>
      <td
        aria-label="21 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        21
      </td>
      <td
        aria-label="22 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        22
      </td>
      <td
        aria-label="23 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        23
      </td>
      <td
        aria-label="24 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        24
      </td>
      <td
        aria-label="25 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        25
      </td>
      <td
        aria-label="26 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        26
      </td>
      <td
        aria-label="27 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        27
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        8
      </td>
      <td
        aria-label="28 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        28
      </td>
      <td
        aria-label="29 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        29
      </td>
    </tr>
  </tbody>
</table>
`;
/* end snapshot vaadin-month-calendar shadow default */

snapshots["vaadin-month-calendar shadow max date"] = 
`<div
  aria-hidden="true"
  id="month-header"
  part="month-header"
>
  February 2016
</div>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat
  as="week"
  style="display: none;"
>
  <template is="dom-repeat">
  </template>
</dom-repeat>
<table
  aria-labelledby="month-header"
  id="monthGrid"
  role="grid"
>
  <thead id="weekdays-container">
    <tr
      part="weekdays"
      role="row"
    >
      <th
        aria-hidden="true"
        hidden=""
        part="weekday"
      >
      </th>
      <th
        abbr="Sunday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Sun
      </th>
      <th
        abbr="Monday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Mon
      </th>
      <th
        abbr="Tuesday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Tue
      </th>
      <th
        abbr="Wednesday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Wed
      </th>
      <th
        abbr="Thursday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Thu
      </th>
      <th
        abbr="Friday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Fri
      </th>
      <th
        abbr="Saturday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Sat
      </th>
    </tr>
  </thead>
  <tbody id="days-container">
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        5
      </td>
      <td
        aria-label=""
        part="date"
        role="gridcell"
        tabindex="-1"
      >
      </td>
      <td
        aria-label="1 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        1
      </td>
      <td
        aria-label="2 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        2
      </td>
      <td
        aria-label="3 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        3
      </td>
      <td
        aria-label="4 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        4
      </td>
      <td
        aria-label="5 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        5
      </td>
      <td
        aria-label="6 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        6
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        5
      </td>
      <td
        aria-label="7 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        7
      </td>
      <td
        aria-label="8 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        8
      </td>
      <td
        aria-label="9 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        9
      </td>
      <td
        aria-label="10 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        10
      </td>
      <td
        aria-disabled="true"
        aria-label="11 February 2016, Thursday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        11
      </td>
      <td
        aria-disabled="true"
        aria-label="12 February 2016, Friday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        12
      </td>
      <td
        aria-disabled="true"
        aria-label="13 February 2016, Saturday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        13
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        6
      </td>
      <td
        aria-disabled="true"
        aria-label="14 February 2016, Sunday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        14
      </td>
      <td
        aria-disabled="true"
        aria-label="15 February 2016, Monday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        15
      </td>
      <td
        aria-disabled="true"
        aria-label="16 February 2016, Tuesday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        16
      </td>
      <td
        aria-disabled="true"
        aria-label="17 February 2016, Wednesday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        17
      </td>
      <td
        aria-disabled="true"
        aria-label="18 February 2016, Thursday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        18
      </td>
      <td
        aria-disabled="true"
        aria-label="19 February 2016, Friday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        19
      </td>
      <td
        aria-disabled="true"
        aria-label="20 February 2016, Saturday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        20
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        7
      </td>
      <td
        aria-disabled="true"
        aria-label="21 February 2016, Sunday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        21
      </td>
      <td
        aria-disabled="true"
        aria-label="22 February 2016, Monday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        22
      </td>
      <td
        aria-disabled="true"
        aria-label="23 February 2016, Tuesday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        23
      </td>
      <td
        aria-disabled="true"
        aria-label="24 February 2016, Wednesday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        24
      </td>
      <td
        aria-disabled="true"
        aria-label="25 February 2016, Thursday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        25
      </td>
      <td
        aria-disabled="true"
        aria-label="26 February 2016, Friday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        26
      </td>
      <td
        aria-disabled="true"
        aria-label="27 February 2016, Saturday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        27
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        hidden=""
        part="week-number"
      >
        8
      </td>
      <td
        aria-disabled="true"
        aria-label="28 February 2016, Sunday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        28
      </td>
      <td
        aria-disabled="true"
        aria-label="29 February 2016, Monday"
        disabled=""
        part="date disabled"
        role="gridcell"
        tabindex="-1"
      >
        29
      </td>
    </tr>
  </tbody>
</table>
`;
/* end snapshot vaadin-month-calendar shadow max date */

snapshots["vaadin-month-calendar shadow week numbers"] = 
`<div
  aria-hidden="true"
  id="month-header"
  part="month-header"
>
  February 2016
</div>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat style="display: none;">
  <template is="dom-repeat">
  </template>
</dom-repeat>
<dom-repeat
  as="week"
  style="display: none;"
>
  <template is="dom-repeat">
  </template>
</dom-repeat>
<table
  aria-labelledby="month-header"
  id="monthGrid"
  role="grid"
>
  <thead id="weekdays-container">
    <tr
      part="weekdays"
      role="row"
    >
      <th
        aria-hidden="true"
        part="weekday"
      >
      </th>
      <th
        abbr="Monday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Mon
      </th>
      <th
        abbr="Tuesday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Tue
      </th>
      <th
        abbr="Wednesday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Wed
      </th>
      <th
        abbr="Thursday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Thu
      </th>
      <th
        abbr="Friday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Fri
      </th>
      <th
        abbr="Saturday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Sat
      </th>
      <th
        abbr="Sunday"
        aria-hidden="true"
        part="weekday"
        role="columnheader"
        scope="col"
      >
        Sun
      </th>
    </tr>
  </thead>
  <tbody id="days-container">
    <tr role="row">
      <td
        aria-hidden="true"
        part="week-number"
      >
        5
      </td>
      <td
        aria-label="1 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        1
      </td>
      <td
        aria-label="2 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        2
      </td>
      <td
        aria-label="3 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        3
      </td>
      <td
        aria-label="4 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        4
      </td>
      <td
        aria-label="5 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        5
      </td>
      <td
        aria-label="6 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        6
      </td>
      <td
        aria-label="7 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        7
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        part="week-number"
      >
        6
      </td>
      <td
        aria-label="8 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        8
      </td>
      <td
        aria-label="9 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        9
      </td>
      <td
        aria-label="10 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        10
      </td>
      <td
        aria-label="11 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        11
      </td>
      <td
        aria-label="12 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        12
      </td>
      <td
        aria-label="13 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        13
      </td>
      <td
        aria-label="14 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        14
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        part="week-number"
      >
        7
      </td>
      <td
        aria-label="15 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        15
      </td>
      <td
        aria-label="16 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        16
      </td>
      <td
        aria-label="17 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        17
      </td>
      <td
        aria-label="18 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        18
      </td>
      <td
        aria-label="19 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        19
      </td>
      <td
        aria-label="20 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        20
      </td>
      <td
        aria-label="21 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        21
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        part="week-number"
      >
        8
      </td>
      <td
        aria-label="22 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        22
      </td>
      <td
        aria-label="23 February 2016, Tuesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        23
      </td>
      <td
        aria-label="24 February 2016, Wednesday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        24
      </td>
      <td
        aria-label="25 February 2016, Thursday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        25
      </td>
      <td
        aria-label="26 February 2016, Friday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        26
      </td>
      <td
        aria-label="27 February 2016, Saturday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        27
      </td>
      <td
        aria-label="28 February 2016, Sunday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        28
      </td>
    </tr>
    <tr role="row">
      <td
        aria-hidden="true"
        part="week-number"
      >
        9
      </td>
      <td
        aria-label="29 February 2016, Monday"
        part="date"
        role="gridcell"
        tabindex="-1"
      >
        29
      </td>
    </tr>
  </tbody>
</table>
`;
/* end snapshot vaadin-month-calendar shadow week numbers */

